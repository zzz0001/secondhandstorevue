<template>
  <div id="goodsDetail">
        ...
        <div class="my-goods-description">
          ...
          <div style="background: ghostwhite;margin-bottom: 14px;margin-top: 4px">
            ...
            <span style="margin-left: 170px;">
              <span v-if="goods.goods.goodsInventory < 1" style="color: red;font-size: 18px;font-weight: 600">缺货</span>
              <span v-else>有货</span>
            </span>
          </div>
          <div>
            <el-button @click="buyGoods" :disabled="goods.goods.goodsInventory < 1" type="primary" style="margin-left: 50px">立即购买</el-button>
            <el-button @click="addOrder(1)" :disabled="goods.goods.goodsInventory < 1" type="danger" style="margin-left: 50px"><i
    ...
    <div v-else>
      <p style="text-align: center;width: 700px;margin: 200px auto auto;font-size: 18px;color: #FF6200;"><i class="el-icon-s-promotion"></i> 该商品不存在，或已被下架，继续浏览其他商品吧 </p>
    </div>
  ...
  methods: {
    ...
    addOrder(status){
      ...
      this.$axios.post('/order',this.order).then(res =>{
        this.order.orderId = res.data.data
